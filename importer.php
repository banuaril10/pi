			...
			<div class="tables">			
				...
				<p id="notif1" style="color: red; font-weight: bold"></p>	
					...
					<table class="table table-bordered" id="example">
						...
						<?php 
						$sql_list = "select a.sku, b.name, sum(a.qty) as quantity, date(a.tanggal) as tgl from inv_temp a left join pos_mproduct b on a.sku = b.sku where a.status = '0' and date(a.tanggal) = date(now()) group by a.sku, b.name, date(a.tanggal)";
						$no = 1;
						...
						?>
							...
							<tr>
								<th scope="row"><input type="checkbox" id="checkbox1" name="checkbox1[]" value="<?php echo $row['sku']; ?>|<?php echo $row['quantity']; ?>|<?php echo $row['tgl']; ?>"></th>
								<td ><?php echo $no; ?></td>
...
function prosesInvtemp(sku, qty, tgl, jumlahpi){

	// alert(sku+' '+qty+ ' '+tgl+' '+jumlahpi);
	$.ajax({
		url: "api/action.php?modul=inventory&act=proses_inv_temp",
		type: "POST",
		data: {sku:sku ,qty:qty, tgl:tgl, jumlahpi:jumlahpi},
		beforeSend:function(){
...
function prosesAll(){
			...
			<!-- alert("approve cuy"); -->
				...
				if(checkboxes.length == 0){
					...
					prosesInvtemp(sku, qty, tgl, jumlahpi);
