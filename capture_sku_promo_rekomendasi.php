...
<body>
  ...
  <script>
    $(document).ready(function () {
      ...
      const apiPromoMaster = "<?php echo $base_url . '/store/rekomendasi_promo/get_promo_master.php'; ?>";
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response.success && Array.isArray(response.data)) {
              var $select = $("#jenis_promo");
              $.each(response.data, function (i, item) {
                $select.append($("<option>", {
                  value: item.promo_name, // kalau mau simpan ID ganti jadi item.id
                  text: item.promo_name
                }));
              });
            } else {
              console.warn("Data promo master kosong atau tidak valid");
            }
          },
          error: function (xhr, status, error) {
            console.error("Gagal memuat data promo master:", error);
          }
        });
      });
