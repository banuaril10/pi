...
// ==== FILTER TANGGAL (BARU) ====
$date_now   = date("Y-m-d"); // default hari ini
$filter_tgl = isset($_GET['tgl']) && !empty($_GET['tgl']) ? $_GET['tgl'] : $date_now;
...
// Ambil data dari API untuk diambil kategorinya — pakai $filter_tgl
$json_url = "https://mkt.idolmartidolaku.com/api/get_sku_plano_multi.php?tgl=".$filter_tgl."&toko=".$value;
			...
			<div class="tables">			
						...
						<div class="row mt-2">
							<div class="col-12">
								<small class="text-muted">
									Menampilkan data planogram tanggal: 
									<b><?= date('d M Y', strtotime($filter_tgl)) ?></b>
									<?php if ($filter_tgl == $date_now) echo "<span class='badge bg-success'>Hari ini</span>"; ?>
								</small>
							</div>
						</div>
					</form>
				...
				<table class="table table-bordered table-striped" id="" style="width: 100%">
					...
					foreach ($arr as $row1) {
						...
						if ($row1['image'] != "")  $img  = $row1['image'];
						if ($row1['image2'] != "") $img2 = $row1['image2'];
						if ($row1['image3'] != "") $img3 = $row1['image3'];
						if ($row1['image4'] != "") $img4 = $row1['image4'];
						if ($row1['image5'] != "") $img5 = $row1['image5'];
						...
						$img_sample4 = "";
						$img_sample5 = "";
						if($row1['file']  != "") $img_sample  = '<img src="'.$row1['base_url'].$row1['file'].'"  style="width: 400px"></img>';
						if($row1['file2'] != "") $img_sample2 = '<img src="'.$row1['base_url'].$row1['file2'].'" style="width: 400px"></img>';
						if($row1['file3'] != "") $img_sample3 = '<img src="'.$row1['base_url'].$row1['file3'].'" style="width: 400px"></img>';
						if($row1['file4'] != "") $img_sample4 = '<img src="'.$row1['base_url'].$row1['file4'].'" style="width: 400px"></img>';
						if($row1['file5'] != "") $img_sample5 = '<img src="'.$row1['base_url'].$row1['file5'].'" style="width: 400px"></img>';
					...
					?>
						...
						<tr>
							...
							<input type="hidden" id="tgl<?php echo $row1['id']; ?>" value="<?php echo $filter_tgl; ?>">
					...
					<?php $no++;} 
						...
						echo '<tr><td colspan="4" class="text-center">Tidak ada data planogram untuk tanggal '.date("d M Y", strtotime($filter_tgl)).'.</td></tr>';
					}
					?>
...
function uploadImage(id, fileType){
	...
	if(vidFileLength === 0){
		...
		var tgl    = $("#tgl"+id).val(); // kirim tanggal juga (opsional)
