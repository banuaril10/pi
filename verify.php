			...
			<div class="tables">		
				...
				<p id="notif" style="color: red; font-weight: bold"></p>
					...
					<table class="table table-bordered" id="example">
						...
						$sql_amount = "select SUM(CASE WHEN issync=1 THEN 1 ELSE 0 END) jumsync,  sum(qtysalesout * price) hargagantung,  sum(qtyerp * price) hargaerp, sum(qtycount * price) hargafisik, count(sku) jumline
...
function releasePI(m_pi){
	...
	$.ajax({
		url: "api/action.php?modul=inventory&act=release",
		type: "POST",
		data : formData,
		processData: false,
		contentType: false,
