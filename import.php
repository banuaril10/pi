...
// {
    ...
    if (!empty($_FILES['import']['name']) && in_array($_FILES['import']['type'], $fileMimes))
    {
             ...
             // Parse data from CSV file line by line
				...
				if($jum > 0){
					 ...
					 while (($getData = fgetcsv($csvFile, 10000, ",")) !== FALSE)
					{
						$sku = $getData[0];
						$qty = $getData[1];
						$import = $connec->query("insert into inv_temp (tanggal, sku, qty, status, filename) VALUES ('".date('Y-m-d')."','".$sku."','".$qty."','0', '".$_FILES['import']['name']."');");
			...
			$json = array("result"=>1, "msg"=>"Berhasil import ".$no." data");
           
         
    }
    else
    {
        $json = array("result"=>0, "msg"=>"Please select valid file");
    }
